...
- [x] **Pixel art sprites for remaining critters**: add sprite sheets (walk1, walk2, idle frames) to `src/sprites.ts` for cat, dog, rabbit, hamster, fox, frog, and turtle. Follow the same pattern as the bird sprite (12x12 pixel color arrays, rendered to canvas). Each critter should have a distinctive colour palette and recognisable silhouette. Keep the same chunky pixel art style. DO NOT modify critter.ts, critterManager.ts, renderer.ts, or main.ts — only add new entries to the SPRITE_SHEETS object in sprites.ts.
- [ ] **Redesign sprites at 24×24 native resolution**: current sprites are 16×16 upscaled 1.5× to the 24px display size, which causes soft/blurry edges from non-integer scaling and leaves very little pixel budget (256 pixels) for detail. Moving to 24×24 native gives crisp 1:1 pixels and 2.25× the pixel budget (576), enough for proper shading, eye highlights, collar/accessory detail, and more expressive faces. `renderFrame()` in `sprites.ts` already handles arbitrary source dimensions — no renderer changes needed, just rewrite each `SpriteFrame` array at 24×24. Use `assets/sprites/reference.png` as the visual target: chibi proportions, bold dark outlines, 2-3 shading tones per colour, expressive faces with eye highlights, and subtle ground shadows. Per-critter notes based on the reference: **cat** sitting pose with curled tail, pink inner ears, white chest/belly, pink nose; **dog** sitting with floppy ears, white muzzle/chest blaze, red collar with gold buckle, tongue out; **hamster** very round, white belly, tiny folded arms, small rounded ears; **bird** (parrot) blue head/wings, yellow body, orange beak, green tail feathers; **rabbit** white with pink inner ears, cottontail; **fox** orange with white chest/cheeks, dark paws, bushy white-tipped tail; **frog** bright green with darker green spots/shading, big round eyes, sitting on grass tuft; **turtle** domed shell with hexagonal pattern, green head with visible eye. Keep walk1/walk2/idle frames per critter.
